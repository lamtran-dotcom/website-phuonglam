...
const HomePage = ({ setPage, addToCart, productImages = {}, featuredIds = null, categoryImages = {}, headerImages = null }) => {
  ...
  const scrollBestsellers = (dir) => {
    ...
    carouselRef.current.scrollBy({ left: dir * 470, behavior: 'smooth' });
  };
  const startCarouselScroll = (dir) => {
    if (carouselIntervalRef.current) return;
    carouselIntervalRef.current = setInterval(() => {
      if (!carouselRef.current) return;
      carouselRef.current.scrollBy({ left: dir * 6, behavior: 'auto' });
    }, 16);
  };
  const stopCarouselScroll = () => {
    ...
    <div>
      ...
      <section style={{ ...hpStyles.section, background: '#fff', padding: isMobile ? '40px 0' : '64px 0' }}>
        <div style={{ maxWidth: 1320, margin: '0 auto', padding: isMobile ? '0 16px' : '0 24px' }}>
          ...
          <div style={hpStyles.carouselWrap}>
            {!isMobile && (
              <button
                style={{ ...hpStyles.carouselBtn, ...hpStyles.carouselBtnLeft }}
                onClick={() => scrollBestsellers(-1)}
                onMouseEnter={() => startCarouselScroll(-1)}
            ...
            <div
              ...
              style={{
                ...hpStyles.carouselTrack,
                gap: isMobile ? 12 : 14,
                padding: isMobile ? '0 0 4px' : '0 54px 4px',
              }}
            >
              {bestsellers.map(p => (
                <div key={p.id} style={{ flex: `0 0 ${isMobile ? 'calc(50% - 6px)' : '220px'}`, scrollSnapAlign: 'start' }}>
                  <ProductCard product={p} setPage={setPage} addToCart={addToCart} productImages={productImages} />
                </div>
              ))}
            </div>
            {!isMobile && (
              <button
                style={{ ...hpStyles.carouselBtn, ...hpStyles.carouselBtnRight }}
                onClick={() => scrollBestsellers(1)}
                onMouseEnter={() => startCarouselScroll(1)}
                onMouseLeave={stopCarouselScroll}
              >›</button>
...
const hpStyles = {
  ...
  carouselTrack: { display: 'flex', overflowX: 'auto', scrollSnapType: 'x mandatory', paddingBottom: 4 },
  carouselBtn: { background: '#fff', border: '1.5px solid #dde8d8', borderRadius: 999, width: 42, height: 42, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26, lineHeight: 1, color: '#318223', cursor: 'pointer', boxShadow: '0 2px 10px rgba(0,0,0,0.08)', transition: 'all .2s', flexShrink: 0, padding: 0, fontWeight: 300 },
  carouselBtnLeft: { position: 'absolute', left: 2, top: '50%', transform: 'translateY(-50%)', zIndex: 3 },
  carouselBtnRight: { position: 'absolute', right: 2, top: '50%', transform: 'translateY(-50%)', zIndex: 3 },
